...
impl ExternalAttractor {
  ...
  pub fn get_external_attraction_acceleration(&self, particle: &Particle, local_density: f32) -> Vector2D<f32> {
    if !self.active {
    ...
    ((vetcor_to_input_point.normalise())/100.0 - particle.velocity * (1.0 - distance_to_input_point / self.radius)/10000.0) / local_density
  }
}
